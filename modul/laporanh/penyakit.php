...
if (!(isset($_SESSION['username']) && isset($_SESSION['password']))) {
  ...
  switch ($_GET[act]) {
    ...
    default:
      ...
      echo "<form method=POST action='?module=laporanh' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
          <br><br><table class='table table-bordered'>
		  <tr><td><input type=text name='keyword' style='margin-left: 10px;' placeholder='Ketik dan tekan cari...' class='form-control kolom-cari' value='$_POST[keyword]' /> <input class='kolom-cari btn bg-olive margin' type=submit value='   Cari   ' name=Go></td> </tr>
      <h2 class='text text-primary'>Laporan Data Hama dan Penyakit &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak data hama dan penyakit'><i class='fa fa-print no-print'></i> Cetak</button> </h2>    
      </table></form>";
      ...
      if ($_POST[Go]) {
        ...
        if ($baris > 0) {
          ...
          if ($offset != 0) {
            ...
            echo "<span class=prevnext> <a href=index.php?module=laporanh&offset=$prevoffset>Back</a></span>";
          } else {
          ...
          for ($i = 1; $i <= $halaman; $i++) {
            $newoffset = $limit * ($i - 1);
            if ($offset != $newoffset) {
              echo "<a href=index.php?module=laporanh&offset=$newoffset>$i</a>";
              //cetak halaman
            } else {
          ...
          if (!(($offset / $limit) + 1 == $halaman) && $halaman != 1) {
            ...
            echo "<span class=prevnext><a href=index.php?module=laporanh&offset=$newoffset>Next</a>";
          } else {
